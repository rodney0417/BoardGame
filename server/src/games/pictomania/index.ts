...
interface PictomaniaPlayer extends Player {
  ...
  myGuesses: { targetPlayerId: string, symbol: string, number: number, time: number }[];
}
...
function loadWords(): { wordCardsByLevel: Record<number, WordCardData[]>, allWordCards: WordCardData[] } {
...
const checkAndFinishRound = (io: Server, room: Room<PictomaniaState, PictomaniaSettings>) => {
    ...
    // 所有人都猜完了，公佈答案並結算
    const allGuesses: { guesserId: string, targetPlayerId: string, symbol: string, number: number, time: number }[] = [];
    ...
    if (roundModel) {
        ...
        room.players.forEach(p => {
         ...
         room.players.forEach(p => {
             ...
             const validHistoryEntry = room.gameState.history.find(h => h.round === room.gameState.currentRound && h.playerId === p.id);
    ...
    io.to(room.id).emit('toast', { type: 'info', message: `第 ${room.gameState.currentRound} 回合結束！公佈答案中...` });
    io.to(room.id).emit('request_round_images', { round: room.gameState.currentRound });
};
...
const Pictomania: GameModule<PictomaniaState, PictomaniaSettings> = {
  ...
  startRound: (room: Room<PictomaniaState, PictomaniaSettings>) => {
    ...
    console.log(`[Pictomania] Starting round ${room.gameState.currentRound}. Players:`, room.players.map(p => ({ id: p.id, username: p.username })));
  ...
  onTimeout: (io, room) => {
    ...
    io.to(room.id).emit('toast', { type: 'info', message: `⏰ 時間到！請把握時間猜題，所有人都猜完後將結束回合。` });
    return true;
  },

  handlers: {
    'start_game': (io: Server, room: Room<PictomaniaState, PictomaniaSettings>, socket: Socket, data: any) => {
      if (room.phase === 'waiting' || room.phase === 'game_over') {
        if (room.players.length < 2) {
          socket.emit('toast', { type: 'error', message: '❌ 至少需要 2 人才能開始遊戲' });
          return false;
        }
        ...
        io.to(room.id).emit('game_started', { cards: room.gameState.wordCards });
      } 
    ...
    'player_finish_drawing': (io, room, socket, data) => {
      ...
      if (player) {
        ...
        if (allDone) {
             io.to(room.id).emit('toast', { type: 'info', message: '🎨 所有人都畫完了，請把握時間猜題！' });
        }
      }
      return true;
    },
    ...
    'guess_word': (io, room, socket, data) => {
      ...
      if (targetPlayer && guesser) {
        if (!guesser.isDoneDrawing) {
          socket.emit('toast', { type: 'error', message: '⚠️ 您必須先點擊「畫好了」才能開始猜題！' });
          return false;
        }
        
        if (guesser.isDoneGuessing) {
           socket.emit('toast', { type: 'error', message: '⚠️ 您已經結束猜題，無法再猜！' });
           return false;
        }
        ...
        if (pIndex !== -1) {
             ...
             p.myGuesses.push({ targetPlayerId, symbol, number, time: Date.now() });
        }
    ...
    'upload_image': (io, room, socket, data) => {
      ...
      if (player && room.gameState.history) {
        const exists = room.gameState.history.some(r => r.round === room.gameState.currentRound && r.playerId === player.id);
        if (!exists) {
            ...
            io.to(room.id).emit('update_canvas', { playerId: player.id, imageBase64 });
            console.log(`[Pictomania] 收到 ${player.username} 第 ${room.gameState.currentRound} 回合的畫作並廣播`);
        }
      }
      return false;
    }
