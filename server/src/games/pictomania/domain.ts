...
export class PictomaniaRound {
    ...
    private titlesForPlayerCount(count: number): number[] {
        ...
        switch (count) {
            case 2: return [3]; // 2-player mode: single high-value card, simpler gameplay
            case 3: return [2, 1];
            case 4: return [2, 1, 1];
            case 5: return [3, 2, 1, 1];
            case 6: return [3, 2, 1, 1, 1];
            default: return Array(Math.max(0, count - 1)).fill(1);
        }
    }
    ...
    public processGuess(guess: GuessResult): number {
        ...
        console.log(`[Domain] processGuess: Target ${guess.targetId} cards: ${JSON.stringify(targetCards)}`);
