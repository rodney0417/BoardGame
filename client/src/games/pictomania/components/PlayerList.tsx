    ...
    <>
      ...
      {otherPlayers.map((p: PictomaniaPlayer) => (
          <Card key={p.id} className="mb-3 border-0 overflow-hidden rounded-4 custom-card shadow-sm">
              <div className="p-3 d-flex justify-content-between align-items-center border-bottom bg-light" style={{ borderTop: `6px solid ${p.color}` }}>
                  <div className="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <Badge bg="info" className="ms-1 rounded-pill text-white">得分: {p.score}</Badge>
                    {p.disconnected && <Badge bg="warning" text="dark" className="ms-1 rounded-pill">斷線</Badge>}
                    {phase === 'round_ended' && (
                        <>
                            <Badge bg="primary" className="ms-1 rounded-pill">題: {p.targetWord}</Badge>
                            {(p.guessedCorrectlyBy as string[])?.includes(me.id) ? 
                                <Badge bg="success" className="ms-1 rounded-pill">✅ 猜對</Badge> : 
                                <Badge bg="danger" className="ms-1 rounded-pill">❌ 猜錯</Badge>
                            }
                        </>
                    )}
                    {phase === 'playing' && p.isDoneDrawing && (
                         <Badge bg="secondary" className="ms-1 rounded-pill">已畫完</Badge>
                    )}
                  </div>
                  {phase === 'playing' && me?.isDoneDrawing && !me?.isDoneGuessing && (
                      <Button 
                        size="sm" 
                        variant={me.myGuesses?.some((g: any) => g.targetPlayerId === p.id) ? "success" : "outline-primary"} 
              ...
              <div className="position-relative bg-white overflow-hidden" style={{ height: '180px' }}>
                  ...
                  {!(phase === 'round_ended' || (phase === 'playing' && me?.isDoneDrawing)) && (
                      <div className="d-flex flex-column align-items-center justify-content-center position-absolute top-0 start-0 w-100 h-100 text-muted bg-light" style={{ zIndex: 5 }}>
