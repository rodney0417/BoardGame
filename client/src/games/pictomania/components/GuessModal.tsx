  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg" className="light-modal">
      ...
      <Modal.Body className="p-4 bg-white text-dark">
        <div className="d-flex justify-content-center mb-4">
          <div className="d-inline-flex gap-2 p-2 bg-light rounded-pill border">
            {Object.keys(cards || {}).length > 0 ? (
              Object.keys(cards || {}).map((s: string) => (
                <Button
                  key={s}
                  ...
                  style={{
                    ...
                    border: selectedSymbol === s ? 'none' : '1px solid transparent',
                  }}
                >
                  <SymbolIcon
                    symbol={s}
        ...
        <Row className="g-3">
          {selectedSymbol && (cards || {})[selectedSymbol] ? (
            (cards || {})[selectedSymbol].map((w: string, i: number) => {
              const number = i + 1;
