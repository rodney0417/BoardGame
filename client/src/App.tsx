...
import { Container, Row, Col, Button, Form, Card, Badge, Navbar, Modal, Toast, ToastContainer } from 'react-bootstrap';
import './index.css';
...
function App() {
  ...
  const [appState, setAppState] = useState<'login' | 'lobby' | 'room'>(savedUsername ? 'lobby' : 'login'); 
  ...
  const [isCheckingAuth, setIsCheckingAuth] = useState<boolean>(!!localStorage.getItem('pictomania_current_room'));
  ...
  useEffect(() => {
    ...
    // Cleanup legacy keys
    const whitelist = ['pictomania_peerId', 'pictomania_username', 'pictomania_current_room', 'pictomania_game_type'];
  ...
  const enterLobby = () => {
    if (!username) return;
    socket.emit('validate_username', { username }, (response: { valid: boolean; message?: string }) => {
      if (response.valid) {
  ...
  const joinRoom = (id: string, gameType: string, color?: string, drawTime?: number) => { 
    ...
    if (username && targetRoomId) { 
      ...
      socket.emit('join_room', { username, roomId: targetRoomId, gameType, color: finalColor, drawTime, peerId }); 
    } 
  };
      ...
      <Container className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh' }}>
        <Card className="custom-card p-5" style={{ maxWidth: '400px', width: '100%' }}>
          ...
          <Form.Group className="mb-4">
            ...
            <Form.Control type="text" size="lg" placeholder="您的名字..." value={username} onChange={e => setUsername(e.target.value)} onKeyPress={(e: any) => e.key === 'Enter' && enterLobby()} />
          </Form.Group>
          <Button className="w-100 py-3 fw-bold rounded-pill" size="lg" onClick={enterLobby} disabled={!username}>進入大廳</Button>
        </Card>
      </Container>
    );
  }
  ...
  return (
    <div className="App" style={{ minHeight: '100vh', paddingBottom: '40px' }}>
      <Navbar className="mb-4" sticky="top">
        <Navbar.Brand className="fw-bold fs-4 ms-2">🪐 {appState === 'lobby' ? '萬遊引力 大廳' : `${activeRoom?.gameName || '未知'} - ${activeRoom?.id}`}</Navbar.Brand>
        <div className="ms-auto d-flex align-items-center gap-3 pe-2">
          <Navbar.Text className="d-none d-sm-block">
            你好, {username}
          </Navbar.Text>
          {appState === 'lobby' && <Button variant="success" size="sm" className="rounded-pill px-3 fw-bold shadow-sm" onClick={() => setShowCreateModal(true)}>創建房間</Button>}
          {appState === 'room' && <Button variant="outline-danger" size="sm" className="rounded-pill px-3 text-nowrap" onClick={leaveRoom}>離開房間</Button>}
        </div>
      </Navbar>
      ...
      <ToastContainer position="top-center" className="p-3" style={{ zIndex: 2000, position: 'fixed' }}>
        {toasts.map(t => (
          <Toast key={t.id} bg={t.type === 'success' ? 'success' : t.type === 'error' ? 'danger' : 'info'} className="custom-toast text-white" autohide delay={3000}>
            <Toast.Body className="fw-bold">{t.type === 'success' ? '🎯 ' : t.type === 'error' ? '❌ ' : 'ℹ️ '}{t.message}</Toast.Body>
          </Toast>
        ))}
      </ToastContainer>
      ...
      <Container>
        {appState === 'lobby' ? (
          <Row className="justify-content-center">
            <Col md={10} lg={8}>
              {roomList.length === 0 ? (
                <Card className="custom-card p-5 border-0 text-center text-muted"><p className="m-0">目前沒有房間，來創一個吧！</p></Card>
              ) : (
                <Row className="g-4">
                  {roomList.map(r => (
                    <Col xl={6} key={r.id}>
                      <Card className="custom-card border-0 h-100" style={{ minHeight: '220px' }}>
                        <Card.Body className="p-4 d-flex flex-column">
                          <div className="d-flex justify-content-between align-items-start mb-3">
                            <div>
                              <h5 className="fw-bold mb-1 text-truncate" style={{ maxWidth: '180px' }}>{r.id}</h5>
                            ...
                            <Badge bg={r.playerCount >= r.maxPlayers ? "danger" : "secondary"} className="rounded-pill px-3">{r.playerCount} / {r.maxPlayers} 人</Badge>
                          </div>
                          {r.gameType === 'pictomania' && (
                            <div className="mb-4 d-flex gap-2">
                              {PLAYER_COLORS.map(c => (
                                <div key={c} style={{ width: '14px', height: '14px', backgroundColor: c, borderRadius: '50%', opacity: r.takenColors?.includes(c) ? 1 : 0.1, border: '1px solid rgba(0,0,0,0.1)' }} />
                              ))}
                            </div>
                          )}
                          <div className="d-grid mt-auto">
                            <Button variant={r.phase === 'playing' || r.playerCount >= r.maxPlayers ? "outline-secondary" : "primary"} disabled={r.phase === 'playing' || r.playerCount >= r.maxPlayers} onClick={() => handleJoinClick(r)} className="fw-bold rounded-pill">
          ...
          (() => {
            ...
            if (!GameComponent || !me) return <div className="text-center py-5 text-white"><h3>載入中...</h3></div>;
      ...
      <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)} centered>
        <Modal.Header closeButton className="border-0 bg-white"><Modal.Title className="fw-bold">創建新房間</Modal.Title></Modal.Header>
      ...
      <Modal show={showTimeModal} onHide={() => setShowTimeModal(false)} centered>
        <Modal.Header closeButton className="border-0 bg-white"><Modal.Title className="fw-bold">設定遊戲規則</Modal.Title></Modal.Header>
        <Modal.Body className="text-center p-4 bg-white">
            <p className="text-muted mb-4">請選擇每一輪的<strong>畫畫時間</strong></p>
            <div className="d-flex justify-content-center gap-3 mb-4">{[30, 60, 90].map(time => (<Button key={time} variant={selectedDrawTime === time ? "primary" : "outline-secondary"} onClick={() => setSelectedDrawTime(time)} className="py-3 px-4 fw-bold rounded-pill">{time}s</Button>))}</div>
            <Button variant="success" size="lg" className="w-100 fw-bold rounded-pill" onClick={() => { setShowTimeModal(false); setShowColorModal(true); }}>確認設定</Button>
        </Modal.Body>
      </Modal>

      <Modal show={showColorModal} onHide={() => setShowColorModal(false)} centered>
        <Modal.Header closeButton className="border-0 bg-white"><Modal.Title className="fw-bold">挑選顏色</Modal.Title></Modal.Header>
        <Modal.Body className="text-center p-4 bg-white">
            <p className="text-muted mb-4">選擇您的<strong>代表色</strong></p>
            <div className="d-flex justify-content-center gap-3 mb-4">
                {PLAYER_COLORS.map(c => {
                  const isTaken = pendingRoom?.takenColors?.includes(c);
                  return (<div key={c} onClick={() => !isTaken && setSelectedColor(c)} style={{ width: '45px', height: '45px', backgroundColor: c, borderRadius: '50%', cursor: isTaken ? 'not-allowed' : 'pointer', border: selectedColor === c ? '4px solid #333' : '2px solid transparent', opacity: isTaken ? 0.2 : 1, transition: 'all 0.2s', position: 'relative' }}>{isTaken && <span style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', color: 'white' }}>✕</span>}</div>);
                })}
            </div>
            <Button variant="primary" size="lg" className="w-100 fw-bold rounded-pill shadow-sm" onClick={() => { if (pendingRoom) joinRoom(pendingRoom.id, pendingRoom.gameType, selectedColor, pendingRoom.isCreating ? selectedDrawTime : undefined); setShowColorModal(false); }}>{pendingRoom?.isCreating ? '建立房間' : '進入房間'}</Button>
        </Modal.Body>
      </Modal>
    </div>
  );
