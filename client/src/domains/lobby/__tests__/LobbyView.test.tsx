...
describe('LobbyView', () => {
  it('renders empty state correctly', () => {
    render(<LobbyView roomList={[]} onJoinRoom={vi.fn()} showCreateModal={false} onCloseCreateModal={vi.fn()} />);
    expect(screen.getByText(/目前沒有房間/i)).toBeInTheDocument();
  });

  it('renders room list', () => {
    render(<LobbyView roomList={mockRooms} onJoinRoom={vi.fn()} showCreateModal={false} onCloseCreateModal={vi.fn()} />);
    expect(screen.getByText('Room1')).toBeInTheDocument();
  });

  it('shows create modal when showCreateModal prop is true', async () => {
    render(<LobbyView roomList={[]} onJoinRoom={vi.fn()} showCreateModal={true} onCloseCreateModal={vi.fn()} />);
